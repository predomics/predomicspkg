                ...
                path = NULL) 
{
  ...
  }else
  {
    ...
    {
      feature.cor     <- filterfeaturesK(data = X, 
                                         trait = y, 
                                         k = max.nb.features, 
                                         type = "wilcoxon",
                                         sort = TRUE, 
  ...
  coeffs          <- getSign(X = X, y = y, clf = clf, parallel.local = FALSE)
  ...
  } else
  {
    ...
    res.clf$crossVal      <- runCrossval(X, y, clf, lfolds = lfolds, nfolds = nfolds, return.all = return.all)
